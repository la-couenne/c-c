...
function love.update(dt)
  ...
  if love.keyboard.isDown("x") then
    ...
    print("** Pr ctrl de l'id de l'unité en 8-12?: "..tostring(UnitsGrid[12][8]))
...
function love.draw()
    ...
    for c=1, MAP_WIDTH do --boucle du nb de colonnes
      ...
      if texQuad ~= nil then -- n'est pas '!=' s'écrit en Lua: '~='
        ...
        if UnitsGrid[l][c] > 0 then -- si ds $UnitsGrid à cette case, j'ai qque chose > 0 alors
          ...
          --print("** Pr ctrl on dessine l'unité "..UnitsGrid[l][c]) -- ### est ce que ds $UnitsGrid on a des 1 ou les id unité?? ###
        end
      end
    end
  end
  ...
  if pointeur.actif == true then
    if pointeur.timer > 0 then
      ...
      love.graphics.draw(TileSheet, TileTextures[pointeur.type], (pointeur.x-CurseurX)*TILE_WIDTH, (pointeur.y-CurseurY)*TILE_HEIGHT)
